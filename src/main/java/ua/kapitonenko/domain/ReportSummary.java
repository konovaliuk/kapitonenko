...
public class ReportSummary {
	private List<ReceiptCalculator> receiptList;
	private List<TaxCategory> taxCats;
	private List<PaymentType> paymentTypes;
	private long noCancelled;
	
	public ReportSummary(List<ReceiptCalculator> receiptList, List<TaxCategory> taxCats, List<PaymentType> paymentTypes) {
	...
	public Map<PaymentType, BigDecimal> costPerPayType() {
		...
		for (ReceiptCalculator receipt : receiptList) {
	...
	public BigDecimal getTaxAmount() {
		return receiptList.stream()
				       .map(ReceiptCalculator::getTaxAmount)
				       .reduce(new BigDecimal("0.00"), BigDecimal::add);
	}
	
	public BigDecimal getTotalCost() {
		return receiptList.stream()
				       .map(ReceiptCalculator::getTotalCost)
				       .reduce(new BigDecimal("0.00"), BigDecimal::add);
